...
import { classifyTerminalEscapeIntroducer } from "./terminal-escape-introducer.js";
...
export function extractPartialEscapeTail(stream: string): string {
  ...
  for (let i = 0; i < stream.length; i++) {
    if (state === "ground") {
      ...
      const escape =
        stream.charCodeAt(i) === ESC ? i : stream.indexOf("\x1b", i);
      if (escape === -1) {
        return "";
      }
      start = escape;
...
export function advancePartialEscapeTail(
  pendingTail: string,
  chunk: string,
): string {
